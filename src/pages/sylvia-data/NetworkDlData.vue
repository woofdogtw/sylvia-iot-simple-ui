...
export default defineComponent({
  ...
  methods: {
    onCsvClick() {
      const url = `${this.$root.config.data.base}/api/v1/network-dldata/list?limit=0&format=csv`;
      const filename = `network-dldata-${this.$root.fileTimeStr(
        new Date()
      )}.csv`;
    ...
    getCount() {
      ...
      let opts = {
        ...
        }/api/v1/network-dldata/count?${qs.stringify(query)}`,
        headers: {
    ...
    getList() {
      ...
      let opts = {
        ...
        url: `${
          this.$root.config.data.base
        }/api/v1/network-dldata/list?${qs.stringify(query)}`,
        headers: {
          Authorization: `Bearer ${tokens.accessToken}`,
        },
      };
