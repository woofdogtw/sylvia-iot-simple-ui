<template>
  ...
  <div class="row items-center">
    <div class="col-xs-3 col-sm-3 col-md-3">{{ $t('appDemo.time') }}</div>
    ...
    <div class="col-xs-4 col-sm-6 col-md-6">{{ $t('appDemo.data') }}</div>
    <div class="col-xs-1 col-sm-1 col-md-1">{{ $t('appDemo.rssi') }}</div>
  </div>
  ...
  <q-list class="padding-zero" no-border highlight separator>
    <q-item v-for="(item, i) in data.list" :key="i">
      ...
      <div class="col-xs-4 col-sm-6 col-md-6">{{ item.data }}</div>
      <div class="col-xs-1 col-sm-1 col-md-1">{{ item.rssi }}</div>
    </q-item>
  </q-list>
</template>
